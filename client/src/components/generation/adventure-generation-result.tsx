...
import type { GeneratedAdventure, AnswerSource } from "@/types";

interface AdventureGenerationResultProps {
...
export function AdventureGenerationResult({
  adventure,
  sources,
  status,
  error,
  ...
  onRegenerate,
  onSave,
}: AdventureGenerationResultProps) {
  if (error) {
    return (
      <ErrorState
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <h3 className="text-base font-semibold text-foreground">Generated Adventure</h3>
          {isStreaming && <Spinner className="h-4 w-4" label="Generating adventure" />}
